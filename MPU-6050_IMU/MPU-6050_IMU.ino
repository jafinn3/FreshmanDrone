...
void write_LCD(){                                                      //Subroutine for writing the LCD
  ...
  if(firstLoop){
    firstLoop = false;
    rollOriginal = angle_roll_buffer % 360;
    pitchOriginal = angle_pitch_buffer % 360; 
  }

  //The light on the MPU6050 needs to be the front of the drone.
  
  int diffRoll = (angle_roll_buffer % 360) - rollOriginal;
  if(abs(diffRoll) > 10){
    int thrustValue = (.5 * abs(diffRoll)) + 5;
    if(diffRoll < 0){
      thrust1 += thrustValue;
      thrust2 -= thrustValue;
      thrust3 -= thrustValue;
  ...
  int diffPitch = (angle_pitch_buffer % 360) - pitchOriginal;
  if(abs(diffPitch) > 10){
    int thrustValue = (.5 * abs(diffPitch)) + 5;
    if(diffPitch < 0){
      thrust1 -= thrustValue;
      thrust2 -= thrustValue;
      thrust3 += thrustValue;
